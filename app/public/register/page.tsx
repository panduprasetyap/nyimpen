...
export default function RegisterPage() {
  ...
  const router = useRouter(); // requires 'import { useRouter } from "next/navigation"'
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // For better UX, we could auto-login, but for now redirect to login
      ...
      <div className="max-w-md w-full">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <button
             type="submit"
             disabled={loading}
             className="block w-full py-4 bg-slate-900 text-white rounded-xl font-semibold text-center hover:bg-slate-800 transition-colors shadow-lg mt-4 disabled:opacity-50"
          >
