  ...
  useEffect(() => {
    async function fetchProfile() {
      try {
        ...
        if (res.ok) {
          const rawData = await res.json(); // Ambil body JSON-nya dulu

          console.log("DATA MENTAH DARI API:", rawData);

          const userData = rawData.data ? rawData.data : rawData;

          setFormData({
            name: userData.name || "",
            email: userData.email || "",
            job_title: userData.job_title || "",
            estimated_monthly_income: userData.estimated_monthly_income
              ? userData.estimated_monthly_income.toString()
              : "",
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden">
        ...
        {notification && (
          <div
            className={`absolute top-0 left-0 right-0 p-4 text-center font-medium text-sm transition-all transform ${
        ...
        <div className="flex flex-col items-center mb-8 mt-2">
          <div className="relative group cursor-pointer">
            <div className="w-24 h-24 rounded-full overflow-hidden bg-slate-100 border-2 border-slate-200">
              {formData.photos ? (
                <img
                  src={`${NEXT_PUBLIC_API_URL}${formData.photos}`}
                  alt="Profile"
